...
public class HomeController extends HttpServlet {
	...
	private void handleError(String errorMsg, String page, HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
	    request.setAttribute("error", errorMsg);
	    Route.forwardToPage(page, request, response);
	}

	private void handleSuccessfulLogin(User user, HttpServletRequest request, HttpServletResponse response) throws IOException {
	    HttpSession session = request.getSession();
	    session.setAttribute("username", user.getUsername());
	    session.setAttribute("role", user.getRole());

	    if ("admin".equals(user.getRole())) {
	        Route.redirectToPage(Route.DASHBOARD, response);
	    } else {
	        Route.redirectToPage(Route.HOME, response);
	    }
	}
	...
	private void registerUser(HttpServletRequest request, HttpServletResponse response) throws Exception {
	    ...
	    if (password.length() < 8 || !password.matches(".*[A-Z].*") || !password.matches(".*[0-9].*")) {
	        handleError("Password must be at least 8 characters long and contain an uppercase letter and a number.", Route.USER_REG, request, response);
	        return;
	    }

	    boolean registered = authService.register(username, password);
	    if (registered) {
	        Route.redirectToPage(Route.USER_LOGIN, response);
	    } else {
